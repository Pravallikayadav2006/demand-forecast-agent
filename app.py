...
functions listed in the "BACKEND CONTRACT" section below. Real backend
functions are provided via adapter.py, which wraps our actual
preprocessing.py / trend_seasonality.py / forecasting.py / anomaly.py /
inventory.py modules to match this contract.
...
BACKEND CONTRACT (share this with your teammate):
    ...
    inventory.py
        recommend(series: pd.DataFrame, forecast_df: pd.DataFrame,
                   lead_time_days: int, service_level_z: float,
                   current_inventory: int) -> dict
            Returns {"safety_stock": float, "reorder_point": float,
                     "reorder_qty": float, "avg_daily_demand": float,
                     "recommended_inventory": float, "current_inventory": int,
                     "shortage_risk": bool, "overstock_risk": bool}
...
if USE_MOCK_BACKEND:
    ...
    def recommend(series, forecast_df, lead_time_days, service_level_z, current_inventory=0):
        ...
        recommended_inventory = reorder_point + reorder_qty
        return {
            ...
            "shortage_risk": current_inventory < reorder_point,
    ...
    from adapter import (
        load_data, get_series, summary_stats, detect_trend,
        detect_seasonality, detect_anomalies, forecast, recommend, generate_insights
    )
# ============================================================
# END BACKEND SWITCH
# ============================================================
...
with st.sidebar:
    ...
    current_inventory = st.number_input("Current inventory (units)", min_value=0, value=200, step=10)
...
def run_pipeline():
    ...
    try:
        result["inventory"] = recommend(
            result["series"], result.get("forecast_df"), lead_time, service_level, current_inventory
...
with tab_inventory:
    ...
    if inventory:
        if inventory.get("shortage_risk"):
            st.error(f"⚠️ SHORTAGE RISK — current inventory ({inventory['current_inventory']} units) is below the reorder point ({inventory['reorder_point']:.0f} units).")
        elif inventory.get("overstock_risk"):
            st.warning(f"⚠️ OVERSTOCK RISK — current inventory ({inventory['current_inventory']} units) is well above the recommended level ({inventory.get('recommended_inventory', 0):.0f} units).")
        else:
            st.success(f"✅ HEALTHY — current inventory ({inventory['current_inventory']} units) is within a safe range.")

        i1, i2, i3, i4, i5 = st.columns(5)
        i1.metric("Current inventory", f"{inventory['current_inventory']}")
        i2.metric("Avg daily demand", f"{inventory['avg_daily_demand']:.1f}")
        i3.metric("Safety stock", f"{inventory['safety_stock']:.0f}")
        i4.metric("Reorder point", f"{inventory['reorder_point']:.0f}")
        i5.metric("Suggested reorder qty", f"{inventory['reorder_qty']:.0f}")
